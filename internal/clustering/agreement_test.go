...
func TestLatentVerdict(t *testing.T) {
	// Aligned: high AMI regardless of shares.
	if v, _ := latentVerdict(0.7, 0.9, 0.4, 5); v != VerdictAligned {
		t.Errorf("high AMI verdict = %q, want %q", v, VerdictAligned)
	}
	// Glued: low AMI, structure far more degenerate than semantics.
	if v, _ := latentVerdict(0.1, 0.55, 0.30, 6); v != VerdictGlued {
		t.Errorf("glued verdict = %q, want %q", v, VerdictGlued)
	}
	// Diverging: low AMI but no dominant structural blob.
	if v, _ := latentVerdict(0.1, 0.35, 0.30, 6); v != VerdictDiverge {
		t.Errorf("diverging verdict = %q, want %q", v, VerdictDiverge)
	}
}
...
// A node the clusterer did not place must be reported as unplaced, not folded
// into cluster 0 — the two mean different things to a client drawing the grid.
func TestLabelVector_UnplacedIsMinusOne(t *testing.T) {
	labels := labelVector([]string{"a", "b", "c"}, map[string]int{"a": 0, "c": 2})
	want := []int{0, -1, 2}
	for i := range want {
		if labels[i] != want[i] {
			t.Fatalf("labelVector = %v, want %v", labels, want)
		}
	}
}
