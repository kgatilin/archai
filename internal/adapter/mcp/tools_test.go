...
func TestToolDefinitions(t *testing.T) {
	...
	// 11 original tools + 5 retrieval tools (search, search_graph, expand, get_node, refresh) + spectral_cluster + components + trophic_layers + semantic_cluster + file_hotspots + latent_domains + embedding_coverage + status + read_file + search_files
	if len(defs) != 26 {
		t.Fatalf("expected 26 tool definitions, got %d", len(defs))
	}
