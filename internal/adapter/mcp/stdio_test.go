...
func TestStdio_ToolsList(t *testing.T) {
	...
	// 11 original tools + 5 retrieval tools + spectral_cluster + components + trophic_layers + semantic_cluster + file_hotspots + latent_domains + embedding_coverage + status + read_file + search_files
	if len(wrapper.Tools) != 26 {
		t.Fatalf("expected 26 tools, got %d", len(wrapper.Tools))
	}
}
