...
func TestStdio_ToolsList(t *testing.T) {
	...
	// 11 original tools + 4 retrieval tools + spectral_cluster + components + trophic_layers + semantic_cluster + file_hotspots + latent_domains + embedding_coverage + status + read_file + search_files
	if len(wrapper.Tools) != 25 {
		t.Fatalf("expected 25 tools, got %d", len(wrapper.Tools))
	}
}
