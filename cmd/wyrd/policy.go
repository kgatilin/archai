...
// newPolicyCmd builds the `wyrd policy` command group. The dependency
// policy is authored under a `policy:` block in wyrd.yaml (see
...
func newPolicyCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Evaluate the dependency policy declared in wyrd.yaml against the
...
authored under a 'policy:' block. See docs/features/dependency-policy.`,
	...
	checkCmd := &cobra.Command{
		...
		Long: `Load the wyrd.yaml overlay, extract the current Go model, and report
...
Packages default to ./... ; pass explicit patterns to scope the scan.`,
	...
	checkCmd.Flags().String("overlay", "", "Path to wyrd.yaml overlay (default: auto-detect wyrd.yaml)")
