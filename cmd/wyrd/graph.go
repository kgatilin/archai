...
// `wyrd graph <tool>`. Each command is named after its MCP tool and
...
// newGraphCmd builds the `wyrd graph` group. The --daemon flag is
...
to reach another. Start a daemon first with 'wyrd daemon start'.`,
	}
